...
% Outputs:                                 
%                                                   
% R = rate   (spikes per sec)                                       
% t = times  (times of rates)                                        
...
m = 1;
D = zeros(size(data)); %spike times for each trial
ND=zeros(1,NT); %trials
for n=1:NT
  indx = find(~isnan(data(n,:)) & data(n,:)>=T(1) & data(n,:)<=T(2)); %spike times in range of T(1)-T(2)
  ND(n) = length(indx); %number of spikes in T time range in trial n
  D(n,1:ND(n)) = data(n,indx);
  m = m + ND(n); 
end
N_tot = m;
N_max = max(ND); %max number of spikes in time range in a single trial
D = D(:,1:N_max);
